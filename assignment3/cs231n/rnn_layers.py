...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
  ...
  next_h = np.tanh(z)
  cache  = (x, Wx, Wh, b, prev_h, next_h, z)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
  return next_h, cache
...
def rnn_step_backward(dnext_h, cache):
  ...
  ##############################################################################
  x, Wx, Wh, b, prev_h, sz, z = cache
  dhdz= 1 - sz**2
  dz  = dhdz * dnext_h
  dWx = x.T.dot(dz)
  dWh = prev_h.T.dot(dz)
  db  = (dhdz * dnext_h).sum(0)
  dprev_h = dz.dot(Wh.T)
  dx  = dz.dot(Wx.T)
  ##############################################################################
  #                               END OF YOUR CODE                             #
...
def rnn_forward(x, h0, Wx, Wh, b):
  ...
  h = np.zeros((N, T, H))
  for t in range(T):
    cur_h, c = rnn_step_forward(x[:,t,:], prev_h, Wx, Wh, b)
    prev_h = cur_h
    h[:,t,:] = cur_h
    cache += [c]
  ##############################################################################
...
def rnn_backward(dh, cache):
  ...
  db  = np.zeros(H)
  for t in range(T)[::-1]:
    dx_t,dprev_h_t,dWx_t,dWh_t,db_t = rnn_step_backward(dh[:,t,:]+dh0, cache[t])
    dx[:,t,:] = dx_t
    dh0 = dprev_h_t
    dWx += dWx_t
    dWh += dWh_t
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
  ...
  cache = (x, Wx, Wh, b, a, next_c, prev_c, prev_h)
  ##############################################################################
  #                               END OF YOUR CODE                             #
  ##############################################################################
...
def lstm_step_backward(dnext_h, dnext_c, cache):
  ...
  #############################################################################
  N, H = dnext_h.shape
  x, Wx, Wh, b, a, next_c, prev_c, prev_h = cache
  ai, af, ao, ag = a[:,:H], a[:,H:(2*H)], a[:,(2*H):(3*H)], a[:,(3*H):]
  i, f, o, g = sigmoid(ai), sigmoid(af), sigmoid(ao), np.tanh(ag)

  do = np.tanh(next_c) * dnext_h
  dnext_c_0 = dnext_c.copy()
